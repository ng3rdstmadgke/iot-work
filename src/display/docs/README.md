...
7セグ表示機は、7つのセグメント(+ドット)にそれぞれGPIOを割り当て、HIGHで表示・LOWで非表示というように表示を切り替える。

![pin_00](img/pin_00.png)

4桁の7セグ表示機のピンは12本。  
一桁でドットも含めて8つのGPIOの割当が必要であれば、4桁だと最低32本必要なのでは？と思うが、この7セグ表示機では一桁ずつ点灯・消灯を高速で繰り返すことによって見た目上4桁表示できる **ダイナミック制御** を採用している。
...
この表示機は **カソードコモン** なので、カソード側のピンは各桁ごとに一つとなっている。
